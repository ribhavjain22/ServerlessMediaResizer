...
def choose_best(input_path, target_bytes):
    ...
    if below_target:
        size, selected_path, selected_meta = max(below_target, key=lambda item: item[0])
        selected_meta["notes"].append("selected the closest result under the requested target")
        return selected_path, selected_meta

    best_effort_meta["notes"].append("target could not be reached without stronger compression")
    return best_effort_path, best_effort_meta
...
def main():
    ...
    selected_path, metadata = choose_best(input_path, target_bytes)
